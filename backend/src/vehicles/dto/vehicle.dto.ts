...
export class CreateVehicleDto {
  ...
  @IsEnum(['motorbike', 'car', 'van', 'truck', 'tuk-tuk']) type: string;
  @ApiProperty() @IsNotEmpty() @IsString() brand: string;
  @ApiProperty() @IsNotEmpty() @IsString() model: string;
  @ApiProperty() @IsNumber() @Min(2000) @Max(2030) @Type(() => Number) year: number;
...
export class UpdateVehicleDto {
  ...
  @IsOptional() @IsEnum(['motorbike', 'car', 'van', 'truck', 'tuk-tuk']) type?: string;
  @IsOptional() @IsString() brand?: string;
  @IsOptional() @IsString() model?: string;
  @IsOptional() @IsNumber() @Min(2000) @Max(2030) @Type(() => Number) year?: number;
  @IsOptional() @IsEnum(['active', 'maintenance', 'inactive']) status?: string;
}
