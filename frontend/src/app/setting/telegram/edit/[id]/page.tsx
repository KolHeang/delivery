...
export default function EditTelegramPage() {
    ...
    const load = async () => {
      try {
        const [configRes, merchantsRes] = await Promise.all([
          api.get(`/telegram/configs/${params.id}`),
          api.get('/select/merchants'),
        ]);

        const mList = Array.isArray(merchantsRes.data)
          ? merchantsRes.data
          : (merchantsRes.data?.result || merchantsRes.data?.results || []);
        setMerchants(mList);

        const cfg = configRes.data;
        if (cfg) {
          setForm({
            merchantId: cfg.merchantId ? String(cfg.merchantId) : '',
            channelTitle: cfg.channelTitle || '',
            chatId: cfg.chatId || '',
            chatType: cfg.chatType || 'group',
            botToken: cfg.botToken || '',
            botUsername: cfg.botUsername || '',
            notifyNewOrder: cfg.notifyNewOrder ?? true,
            notifyDeliverySuccess: cfg.notifyDeliverySuccess ?? true,
            notifyDeliveryFailed: cfg.notifyDeliveryFailed ?? true,
            notifySettlement: cfg.notifySettlement ?? true,
            isActive: cfg.isActive ?? true,
          });
        }
      } catch (err) {
        alert(isKh ? 'មិនអាចទាញយកទិន្នន័យបានទេ' : 'Failed to load Telegram configuration');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.patch(`/telegram/configs/${params.id}`, payload);
      router.push('/setting/telegram');
    } catch (err: any) {
      alert(err.response?.data?.message || (isKh ? 'បរាជ័យក្នុងការកែប្រែ Telegram' : 'Error updating Telegram config'));
    }
    setSaving(false);
  };
    ...
    return (
      <div className="app-layout">
        ...
        <div className="main-content">
          <Topbar
            title={isKh ? 'កែសម្រួល Telegram' : 'Edit Telegram'}
            subtitle={isKh ? 'កំពុងទាញយកទិន្នន័យ...' : 'Loading details...'}
          />
  ...
  return (
    <div className="app-layout">
      ...
      <div className="main-content">
        <Topbar
          title={isKh ? 'កែសម្រួល Telegram' : 'Edit Telegram'}
          subtitle={isKh ? 'កែប្រែ និងកំណត់ការជូនដំណឹង Telegram' : 'Update Telegram notification settings'}
        />

        <div className="page-content">
          <div className="card">
            <div className="card-header">
              <span className="card-title">{isKh ? 'កែសម្រួល Telegram' : 'Edit Telegram'}</span>
            </div>
